...
class FewShotDataset(Dataset):
    def __init__(self, full_dataset, n_shot, transform):
        self.full_dataset = full_dataset
        self.targets = [label for _, label in self.full_dataset]
        self.indices = self._extract_n_shot_indices(n_shot)
        self.transform = transform

    def _extract_n_shot_indices(self, n_shot):
        # 假设self.full_dataset有一个targets属性，包含所有样本的类标签
        ...
        for cls in unique_classes:
            ...
            if len(cls_indices) < n_shot:
                selected_indices = np.random.choice(cls_indices, n_shot, replace=True)
            else:
                selected_indices = np.random.choice(cls_indices, n_shot, replace=False)
...
class ClassificationDataset:
    def __init__(
            ...
            preprocess,
            val_transform,
            location='/home/yichengx/vlmcl/zscl/datasets/',
            batch_size=128,
    ...
    def build_dataloader(self):
        if self.num_shots == -1:
            ...
            self.fs_train_dataset = FewShotDataset(full_dataset=self.train_dataset, transform=self.preprocess,
...
class MNIST(ClassificationDataset):
    def __init__(self, *args, **kwargs):
        # Extract data_root before passing to parent
        data_root = kwargs.pop('data_root', None)
        super().__init__(*args, **kwargs)
        self.name = "mnist"
        # Use data_root if provided, otherwise use self.location
        location = data_root if data_root is not None else self.location
        self.train_dataset = datasets.MNIST(
            location, train=True, download=True, transform=self.preprocess
        )
        self.test_dataset = datasets.MNIST(
            location, train=False, download=True, transform=self.val_transform
        )
...
class CIFAR100(ClassificationDataset):
    def __init__(self, *args, **kwargs):
        ...
        # Use data_root if provided, otherwise use self.location
        location = data_root if data_root is not None else self.location
        dataset = cifar100(preprocess=self.preprocess, val_transform=self.val_transform, location=location)
...
#         )
#         self.fs_train_dataset = FewShotDataset(self.train_dataset, n_shot=8, transform=self.preprocess)
...
#
#         self.train_dataset = self.generate_fewshot_dataset(train_dataset)
...
#
#         self.classnames = [
#             "annual crop land satellite location",
#             "forest satellite location",
#             "brushland or shrubland satellite location",
#             "highway or road satellite location",
#             "industrial buildings or commercial buildings satellite location",
#             "pasture land satellite location",
#             "permanent crop land satellite location",
#             "residential buildings or homes or apartments satellite location",
#             "river satellite location",
#             "lake or sea satellite location",
#         ]
#
#         self.templates = [
