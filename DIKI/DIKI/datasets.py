...
def get_dataset(cfg, split, transforms=None):
    ...
    if cfg.dataset == "MTIL":
        ...
        '''
        if cfg.num_shots >= 1:
            base_sets = [Aircraft, Caltech101, DTD, EuroSAT, Flowers, Food, MNIST, OxfordPet, StanfordCars, SUN397]
        else:
            if cfg.MTIL_order_2:
                # base_sets = [StanfordCars, Food, MNIST, OxfordPet, Flowers, SUN397, Aircraft, Caltech101, DTD, EuroSAT]
                base_sets = [Aircraft, Caltech101, DTD, EuroSAT, Flowers, Food, MNIST, OxfordPet, StanfordCars, SUN397]
            else:
                base_sets = [Aircraft, Caltech101, DTD, EuroSAT, Flowers, Food, MNIST, OxfordPet, StanfordCars, SUN397]
...
def parse_sample(sample, is_train, task_id, cfg):
    return sample[0], sample[1], sample[2], torch.IntTensor([task_id]).repeat(sample[0].size(0))