...
from .utils import MRR_Acc, cls_acc, cosine_loss_3d, cal_MTIL_metrics, ranked_acc, Cosine_Score
    ...
    def __init__(self, cfg, classnames, templates, clip_model):
        ...
        if isinstance(classnames[0], list):
            ...
            for idx, cls_name in enumerate(classnames):
                ...
                self.base_classnames.extend(cls_name)
...
class DIKI:
    def __init__(self, cfg, device, classes_names, templates, load_file=None):
        ...
        self.csv_path = os.path.join(os.getcwd(), "semantics_DIKI.csv")
    ...
    # def get_local_base_classnames(self, task_id):
    #     pp = (
    #         self.model.module.prompt_processor
    #         if isinstance(self.model, torch.nn.DataParallel)
    #         else self.model.prompt_processor
    #     )

    #     class_idx = pp.class_ids_per_task[task_id]
    #     return [pp.base_classnames[i] for i in class_idx]
    def get_local_base_classnames(self, task_id):
        pp = (
            self.model.module.prompt_processor
            if isinstance(self.model, torch.nn.DataParallel)
        ...
        class_idx = pp.class_ids_per_task[task_id]
        local_base = [pp.base_classnames[i] for i in class_idx]

        if task_id == 3:
            local_base = [f"{name} satellite location" for name in local_base]
        elif task_id == 6:
            local_base = [f'number: "{name}"' for name in local_base]
        # breakpoint()
        return local_base
    ...
    def train_one_task(self, cfg, task_id, datasets, metric_logger):
        ...
        covar = covar +  torch.eye(covar.size(0), device=covar.device, dtype=torch.float)*1e-7  # to avoid precision problems
        self.model_wo_dp.means[task_id] = mean
        self.model_wo_dp.covars[task_id] = covar
        self.model_wo_dp.task_learnt = self.model_wo_dp.task_learnt + 1
        ...
        for epoch in tqdm(range(cfg.DIKI.optim.max_epoch)):
            ...
            for idx, sample in enumerate(train_loader):
                ...
                main_loss_tot = loss_main.item() + main_loss_tot
                loss_num = 1 + loss_num
            ...
            if val_dataset:
                ...
                with torch.no_grad():
                    for sample in val_loader:
                        ...
                        res = self.model(inputs, task_ids)
                        outputs = res["outputs"]
                        cur_right = cur_right + torch.sum((outputs.argmax(dim=1)==targets))
                        cur_all = cur_all + targets.size(0)
    ...
    def eval_all(self, cfg, datasets, metric_logger, metric_writer, acc_list):
        ...
        for cur_task in tqdm(range(cfg.nb_task)):
            ...
            metrics = self.evaluate(cfg, eval_loader, cur_task, metric_logger)
            print(f"Task {cur_task} metrics:", metrics)
    ...
    # def evaluate(self, cfg, loader, task_id, classnames, metric_logger=None):
    #     self.model.eval()
    #     store = self.init_metric_store()
    #     right_num = 0
    #     sample_num = 0
    #     with torch.no_grad():
    ...
    #             if metric_logger:
    #                 metric_logger.add([outputs.cpu().argmax(dim=1), targets.cpu(), task_ids], subset="test")
    #             # right_num = right_num + torch.sum(outputs.argmax(dim=1) == targets).item()
    #             # sample_num = sample_num + inputs.size(0)
    #             # top-k accuracy
    #             t1, t3, t5 = cls_acc(outputs, targets, topk=(1, 3, 5))
    #             store["top1"] += t1
    #             store["top3"] += t3
    #             store["top5"] += t5
    #             # classname-based metrics
    #             r3, r5 = ranked_acc(outputs, targets, classnames, topk=(3, 5))
    #             store["ranked3"] += r3
    #             store["ranked5"] += r5

    #             store["mrr"] += MRR_Acc(outputs, targets, classnames)
    #             store["cosine"] += Cosine_Score(outputs, targets, classnames)
    ...
    def evaluate(self, cfg, loader, task_id, metric_logger=None):
        ...
        # task-local classnames
        local_base_classnames = self.get_local_base_classnames(task_id)

        with torch.no_grad():
            for sample in loader:
                inputs, targets, impaths, task_ids = parse_sample(
                    sample, is_train=False, task_id=task_id, cfg=cfg
                )
                ...
                # IMPORTANT: pass task_ids
                res = self.model(inputs, task_ids)
                outputs = res["outputs"]          # [B, C_task]
                B = targets.size(0)
                
                # Logger (task-local accuracy)
                if metric_logger:
                    metric_logger.add(
                        [outputs.argmax(1).cpu(), targets.cpu(), task_ids],
                        subset="test"
                    )

                # ---- metrics (ALL task-local) ----
                t1, t3, t5 = cls_acc(outputs, targets, topk=(1, 3, 5))
                store["top1"] += t1
                store["top3"] += t3
                store["top5"] += t5
                r3, r5 = ranked_acc(
                    outputs, targets, local_base_classnames, topk=(3, 5)
                )
                # top3_values, top3_indices = outputs.topk(k=3, dim=1)
                ...
                #     pred_idx = top3_indices[b].tolist()
                #     gt_name = local_base_classnames[targets[b].item()]
                    
                #     # Load rankings (assuming it's available from utils)
                #     from DIKI.utils import rankings
                #     GT_rank_list = rankings[gt_name][:3]
                #     gt_rank_map = {item["class"]: item["rank"] - 1 for item in GT_rank_list}
                    
                #     pred_classes = [local_base_classnames[i] for i in pred_idx]
                ...
                #     writer = csv.writer(f)
                #     for idx, (s, impath, g) in enumerate(zip(per_sample_scores, impaths, targets.tolist())):
                #         if 0 < s < 1:
                #             top3_preds = [local_base_classnames[top3_indices[idx][i].item()] for i in range(3)]
                #             gt_name = local_base_classnames[g]
                #             writer.writerow([impath, s, gt_name] + top3_preds)
                store["ranked3"] += r3
                store["ranked5"] += r5

                store["mrr"] += MRR_Acc(
                    outputs, targets, local_base_classnames
                )

                store["cosine"] += Cosine_Score(
                    outputs, targets, local_base_classnames
                )
