...
class Evaluator:
    ...
    def __init__(self, cfg):
        ...
        self.reset([0])
    def load_ranks(self):
        rank_path = '/HDD/Suvam_Backup/Suvam/rankings.json'
        if not os.path.exists(rank_path):
            print(f"Rankings file not found at {rank_path}")
            return
        with open(rank_path, 'r') as f:
            self.rank_data = json.load(f)
        print("Rankings loaded successfully.")
        # Sample keys for debugging
        sample_keys = list(self.rankings_raw.keys())[:3]
        print(f"   Sample class names: {sample_keys}")
